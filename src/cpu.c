...
bool emulate_i8080(State8080* state, IOInterface* io,
    ...
    if ( abs(last_sp - state->sp) > 2) {
        printf("\nStack Squash?\n\nPC: 0x%.4x\nLast SP: 0x%.4x\nCurrent SP:
    0x%.4x\n\n", state->pc, last_sp, state->sp); last_sp = state->sp;
    }
    */
    ...
    case 0x92:
        ...
        break; // SUB D
    case 0x93:
        subtract_to_accumulator(state, state->e);
        state->t_states += 4;
        break; // SUB E
    case 0x94:
    ...
    case 0xce: { // ACI D8
        uint16_t result =
            (uint16_t)state->a + (uint16_t)opcode[1] + (uint16_t)state->cc.cy;
        state->cc.z = (result & 0xff) == 0;
        state->cc.s = (result & 0x80) != 0;
        state->cc.cy = result > 0xff;
    ...
    case 0xde: { // SBI D8
        ...
        state->cc.ac =
            ((state->a & 0x0f) - (opcode[1] & 0x0f) - state->cc.cy) < 0;
        state->cc.p = parity(result & 0xff);
        state->a = result & 0xff;
        state->pc += 1;
        state->t_states += 7;
...
void adc(State8080* state, uint8_t num) {
    uint16_t result =
        (uint16_t)state->a + (uint16_t)num + (uint16_t)state->cc.cy;
